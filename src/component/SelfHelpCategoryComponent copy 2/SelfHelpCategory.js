...
function SelfHelpCategory(){
   const [blogs, setBlogs] = useState([]);
    const [message, setMessage]=useState("");

    useEffect(() => {
     axios.get("http://localhost:3001/blog/fetch?category=Self-Help")
            .then(res => setBlogs(res.data))
            .catch(err => console.error("Failed to fetch self-help blogs", err));
    }, []);
...
export default SelfHelpCategory;